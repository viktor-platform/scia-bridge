...
def create_visualization_little_red_cars(params):
    ...
    for y_car in y_cars[1:-1]:
        car_obj = Extrusion(car_points_base, Line(
            Point(length - talud_x_width - 8, y_car, 0.3),
            Point(length - talud_x_width - 8, y_car, 2)
        ))
        car_obj.material = car_material
        geometry_group.add(car_obj)

        car_obj = Extrusion(car_points_roof, Line(
            Point(length - talud_x_width - 8, y_car + 1, 2),
            Point(length - talud_x_width - 8, y_car + 1, 3)
        ))
        car_obj.material = car_material
        geometry_group.add(car_obj)
        ...
        for y_wheel in [y_car + 1.5, y_car + 5.5]:
            wheel_obj = CircularExtrusion(1, Line(
                Point(length - talud_x_width - 8.1, y_wheel, 0.5),
                Point(length - talud_x_width - 4.4, y_wheel, 0.5)
            ))
    ...
    for y_car in y_cars[1:-1] + 20:
        car_obj = Extrusion(car_points_base, Line(
            Point(talud_x_width + 4.5, y_car, 0.3),
            Point(talud_x_width + 4.5, y_car, 2)
        ))
        car_obj.material = car_material
        geometry_group.add(car_obj)

        car_obj = Extrusion(car_points_roof,
                            Line(Point(talud_x_width + 4.5, y_car + 2, 2), Point(talud_x_width + 4.5, y_car + 2, 3)))
        car_obj.material = car_material
        geometry_group.add(car_obj)

        for y_wheel in [y_car + 1.5, y_car + 5.5]:
            wheel_obj = CircularExtrusion(1, Line(
                Point(talud_x_width + 8.1, y_wheel, 0.5),
                Point(talud_x_width + 4.4, y_wheel, 0.5)
            ))
            wheel_obj.material = Material('wheel', threejs_roughness=1, color=Color.black())
            geometry_group.add(wheel_obj)
